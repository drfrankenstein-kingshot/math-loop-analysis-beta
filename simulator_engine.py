...
# =========================================================================
# --- CORE ENGINE STRUCTS & CLASSES ---
# =========================================================================
class TroopSide:
    def __init__(self, troops, stats, leader_heroes, supporter_heroes, tg3_ratio=None):
        self.troops = np.array(troops, dtype=float)          # [Inf, Cav, Arch]
        self.stats = np.array(stats, dtype=float)            # 3x4 matrix
        self.leader_heroes = list(leader_heroes)
        self.supporter_heroes = list(supporter_heroes)
        
        if tg3_ratio is None:
            self.tg3_ratio = np.array([1.0, 1.0, 1.0])
        elif isinstance(tg3_ratio, (int, float)):
            self.tg3_ratio = np.array([tg3_ratio, tg3_ratio, tg3_ratio])
        else:
